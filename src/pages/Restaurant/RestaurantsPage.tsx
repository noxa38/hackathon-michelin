...
import { Search, ChevronDown, ChevronLeft, ChevronRight, X, Utensils, Award, Coins, Globe, Heart } from 'lucide-react'
...
function getRestaurantCountry(restaurant: Restaurant): string {
  if (restaurant.country?.trim()) return restaurant.country.trim()
  if (restaurant.location?.trim()) return extractCountry(restaurant.location)
  return ''
}
...
type OpenMenu = 'distinction' | 'cuisine' | 'price' | 'country' | null
...
export default function RestaurantsPage() {
  ...
  const filtered = useMemo(() => {
    return restaurants.filter(r => {
      ...
      if (countryFilters.length > 0) {
        if (!countryFilters.includes(getRestaurantCountry(r))) return false
      }
      if (search.trim()) {
  ...
  }, [restaurants, distinctionFilters, cuisineFilters, priceFilters, countryFilters, search])
  ...
  useEffect(() => {
    ...
    setCurrentPage(1)
  }, [search, distinctionFilters, cuisineFilters, priceFilters, countryFilters])
  ...
  const hasFilters = distinctionFilters.length + cuisineFilters.length + priceFilters.length + countryFilters.length > 0 || search.trim().length > 0

  function resetAll() {
    setDistinctionFilters([]); setCuisineFilters([]); setPriceFilters([])
    setCountryFilters([]); setSearch('')
  }
